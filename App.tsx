...
import { fetchProductsFromMedusa, getOrCreateCart, addToMedusaCart } from './services/medusaService';
...
const App: React.FC = () => {
  ...
  const handleAddToCart = async (product: Product) => {
    ...
    // 2. Real Backend Sync (Agentic Checkout Flow)
    if (cartId && product.variantId) {
      try {
        await addToMedusaCart(cartId, product.variantId, 1);
        console.log("Arch & Grain: Item added to Medusa Cart", product.variantId);
      } catch (e) {
        console.error("Arch & Grain: Failed to sync with Medusa Cart", e);
      }
    } else {
      console.warn("Arch & Grain: Cart ID or Variant ID missing. Item not synced to backend.");
    }
  };
